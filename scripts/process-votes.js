...
async function main() {
    try {
        ...
        if (issues.length === 0) {
            console.log('✓ No votes to process - exiting normally');
            return;
        }
        ...
        for (const invalid of invalidIssues) {
            try {
                if (invalid.comment) {
                    ...
                    console.log(`✓ Added comment to #${invalid.number}`);
                }
                await closeIssue(invalid.number, 'not_planned');
                console.log(`✓ Closed #${invalid.number}: ${invalid.reason}`);
            } catch (e) {
                console.error(`✗ Failed to close #${invalid.number}:`, e.message);
            }
        }
        ...
        for (const duplicate of duplicateIssues) {
            try {
                if (duplicate.comment) {
                    ...
                    console.log(`✓ Added comment to #${duplicate.number}`);
                }
                await closeIssue(duplicate.number, 'not_planned');
                console.log(`✓ Closed #${duplicate.number}: Duplicate vote`);
            } catch (e) {
                console.error(`✗ Failed to close #${duplicate.number}:`, e.message);
            }
        }
