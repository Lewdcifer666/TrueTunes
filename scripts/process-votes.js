...
// ===== ADMIN CONFIGURATION =====
// Admin users - immune to rate limits for testing purposes
const ADMIN_USERS = ['Lewdcifer666'];

// Trusted app identifier - only votes with this signature are counted
const TRUSTED_APP_SIGNATURE = 'Auto-generated by TrueTunes extension';

// ===== SECURITY SETTINGS =====
// Require votes to come from the extension (blocks manual GitHub submissions)
const REQUIRE_EXTENSION_SIGNATURE = true;

// Minimum account age (in days) to prevent fresh spam accounts
...
function parseVote(issue) {
    ...
    // Check if vote came from the trusted extension
...
async function validateVote(vote) {
    ...
    if (ADMIN_USERS.includes(vote.reporter)) {
        console.log(`✓ [ADMIN] ${vote.reporter} - bypassing all checks`);
        return { valid: true, issues: [] };
    }

    // Check if vote came from extension
    ...
    try {
        ...
        issues.push('Could not verify account age');
    }
...
async function main() {
    ...
    console.log('TrueTunes Vote Processor');
    console.log('='.repeat(60));
    console.log(`Admins: ${ADMIN_USERS.join(', ')}`);
    console.log(`Extension signature required: ${REQUIRE_EXTENSION_SIGNATURE}`);
    ...
    const invalidIssues = [];    // Issues that failed validation
    const duplicateIssues = [];  // Duplicate votes from same user
    const thresholdIssues = [];  // Issues for artists that reached 10 votes
    ...
    for (const issue of issues) {
        ...
        if (!vote) {
            console.log(`❌ Issue #${issue.number}: Invalid format`);
        ...
        console.log(`   From extension: ${vote.isFromExtension ? '✓' : '✗'}`);
        ...
        // Initialize artist data if needed
    ...
    // Move artists to flagged if they hit threshold
    ...
    pending.artists = pending.artists.filter(artist => {
        if (artist.votes >= MIN_VOTES) {
            ...
            // Find all issues for this artist
    ...
    // Close invalid issues with comments
    ...
    for (const invalid of invalidIssues) {
        ...
        console.log(`   ✓ Closed #${invalid.number}: ${invalid.reason}`);
    }
    ...
    for (const duplicate of duplicateIssues) {
        ...
        console.log(`   ✓ Closed #${duplicate.number}: Duplicate vote`);
    }

    // Close threshold issues
    console.log('\n📌 Closing threshold-reached issues...');
